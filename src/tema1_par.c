...
void *func(void *argums) {
    // extract args_wrapper fields
    arg_func_wrapper *args_wrapper = (arg_func_wrapper *)argums;
    arg_func *args = args_wrapper->args;
    long id = args_wrapper->id;
    ...
    if (image->x > RESCALE_X || image->y > RESCALE_Y) {
        ...
        // alloc memory for image only one time
        if(id == 0) {
            args->new_image = (ppm_image *)malloc(sizeof(ppm_image));

            if (!args->new_image) {
                fprintf(stderr, "Unable to allocate memory\n");
                exit(1);
            }
            args->new_image->x = RESCALE_X;
            args->new_image->y = RESCALE_Y;

            args->new_image->data = (ppm_pixel*)malloc(args->new_image->x * args->new_image->y * sizeof(ppm_pixel));
            if (!args->new_image) {
                fprintf(stderr, "Unable to allocate memory\n");
                exit(1);
            }
        ...
        start = id * args->new_image->x / P;
        end = min((id + 1) * args->new_image->x / P, args->new_image->x);
    ...
    // sample_grid function ///////////////////////////////////////////////////////////////////////////////////////

    // alloc memory for grid only one time
    if(id == 0) {
...
int main(int argc, char *argv[]) {
    ...
    int P = atoi(argv[3]); // number of threads

    // initialize arguments for function
    arg_func args;
    ...
    pthread_barrier_init(&args.barrier, NULL, P); // barrier initialization

    // initialize wrapper fields for function (id and args)
    arg_func_wrapper *args_wrapper = (arg_func_wrapper *)malloc(P * sizeof(arg_func_wrapper));

    int i;
    for(i = 0; i < P; i++) 
        args_wrapper[i].args = &args;
    ...
    // create threads
    for (id = 0; id < P; id++) {
        args_wrapper[id].id = id;

        r = pthread_create(&threads[id], NULL, func, (void *) &args_wrapper[id]);
 
        if (r) {
            fprintf(stderr, "Eroare la crearea thread-ului %ld\n", id);
            exit(-1);
        }
    }
    ...
    for (id = 0; id < P; id++) {
        ...
        if (r) {
            fprintf(stderr, "Eroare la asteptarea thread-ului %ld\n", id);
            exit(-1);
        }
    }
    ...
    // 3. March the squares
    // march(args.image, grid, contour_map, step_x, step_y);

    // 4. Write output
    write_ppm(args.image, argv[2]);

    free_resources(args.image, contour_map, grid, step_x);
